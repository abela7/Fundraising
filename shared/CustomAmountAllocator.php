...
class CustomAmountAllocator {
    ...
    /**
     * Undo/reverse custom amount allocation when unapproving
     */
    public function undoCustomAmount(int $pledgeId, float $amount, string $donorName): array {
        try {
            $this->db->begin_transaction();
            
            // First, deallocate any floor cells allocated to this pledge
            $gridAllocator = new IntelligentGridAllocator($this->db);
            $deallocationResult = $gridAllocator->deallocate($pledgeId, null);
            
            // Then, reverse the custom amount tracking
            // If this was under £100, it was only tracked, so subtract from tracking
            if ($amount < 100) {
                $update = $this->db->prepare("
                    UPDATE custom_amount_tracking 
                ...
                error_log("CustomAmountAllocator: Reversed tracking of £{$amount} for {$donorName}");
            } else {
                // For £100+ amounts, we need to reverse both allocated cells AND any remaining tracked amount
                $cellsAllocated = $this->calculateCellsForAmount($amount);
                $allocatedAmount = $cellsAllocated * 100;
                $remainingAmount = $amount - $allocatedAmount;
                
                if ($remainingAmount > 0) {
                    // Reverse the remaining amount tracking
                    $update = $this->db->prepare("
                        UPDATE custom_amount_tracking 
                        SET total_amount = GREATEST(0, total_amount - ?),
                            remaining_amount = GREATEST(0, remaining_amount - ?),
                        ...
                        WHERE id = 1
                    ");
                    $update->bind_param('dd', $remainingAmount, $remainingAmount);
                    $update->execute();
                    
                    error_log("CustomAmountAllocator: Reversed remaining £{$remainingAmount} for {$donorName}");
                }
            }
            ...
            return [
                'success' => true,
                'message' => "Successfully reversed custom amount allocation for £{$amount}",
                'deallocation_result' => $deallocationResult
            ];
            
        } catch (Exception $e) {
            $this->db->rollback();
            error_log("CustomAmountAllocator Undo Error: " . $e->getMessage());
    ...
    /**
     * Undo payment custom amount allocation
     */
    public function undoPaymentCustomAmount(int $paymentId, float $amount, string $donorName): array {
        try {
            $this->db->begin_transaction();
            
            // First, deallocate any floor cells allocated to this payment
            $gridAllocator = new IntelligentGridAllocator($this->db);
            // CRITICAL FIX: Pass the paymentId correctly, not a pledgeId
            $deallocationResult = $gridAllocator->deallocate(null, $paymentId);
            
            // Then, reverse the custom amount tracking
            // Use the provided donorName for logging and potential future logic
            error_log("CustomAmountAllocator: Reversing tracking for payment ID {$paymentId}, amount £{$amount}, donor: {$donorName}");
            ...
            return [
                'success' => true,
                'message' => "Successfully reversed payment custom amount allocation for £{$amount}",
                'deallocation_result' => $deallocationResult
            ];
            
        } catch (Exception $e) {
            $this->db->rollback();
            error_log("CustomAmountAllocator Payment Undo Error: " . $e->getMessage());
