...
// AJAX endpoints (same file for simplicity)
if (isset($_GET['action'])) {
    ...
    if ($action === 'recipients') {
        ...
        while ($row = $q->fetch_assoc()) {
            $out[] = ['id'=>(int)$row['id'], 'name'=>$row['name'], 'role'=>$row['role']];
        }
        jres(['recipients'=>$out]);
    }
	if ($action === 'conversations') {
		// Latest message per pair the user is in
		$sql = "
	...
	if ($action === 'messages') {
        ...
        // Mark read for messages addressed to me
		...
		// Fetch messages
		if ($afterId > 0) {
			$sql = 'SELECT id, sender_user_id, recipient_user_id, body, read_at, created_at FROM user_messages WHERE pair_min_user_id=? AND pair_max_user_id=? AND id>? ORDER BY id ASC';
			$stmt = $db->prepare($sql);
			$stmt->bind_param('iii', $min, $max, $afterId);
		} else {
			$sql = 'SELECT id, sender_user_id, recipient_user_id, body, read_at, created_at FROM user_messages WHERE pair_min_user_id=? AND pair_max_user_id=? ORDER BY id DESC LIMIT 100';
			$stmt = $db->prepare($sql);
		...
		// If initial load (no after), reverse to oldest-first
	...
	if ($action === 'send' && $_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		// Blocklist check both directions
...
<head>
  ...
  <link rel="stylesheet" href="assets/messages-modern.css?v=<?php echo time(); ?>">
</head>
<body>
...
<script src="assets/messages-modern.js?v=<?php echo time(); ?>"></script>
</body>
</html>
