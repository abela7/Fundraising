...
$records_per_page = 25; // Sensible default
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($action, ['approve_payment','reject_payment','update_payment'], true)) {
        ...
        // Preserve the page number after an action
        header('Location: index.php?msg=' . urlencode($actionMsg) . '&page=' . $page);
        exit;
    }
...
$countSql = "
    SELECT
        (SELECT COUNT(*) FROM pledges WHERE status = 'pending') +
        (SELECT COUNT(*) FROM payments WHERE status = 'pending') as total_count
";
$total_records = (int)($db->query($countSql)->fetch_assoc()['total_count'] ?? 0);
