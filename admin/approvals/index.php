...
// AJAX form handling for instant approvals/rejections
$(document).ready(function() {
    $('.ajax-form').on('submit', function(e) {
        ...
        const loadingText = $button.data('loading-text') || '<i class="fas fa-spinner fa-spin"></i>';
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    const $item = $('#pledge-' + itemId);
                    $item.fadeOut(300, function() {
                        $(this).remove();
                        
                        // Check if list is empty
                        if ($('.approval-list .approval-item').length === 0) {
                            $('.approval-list').html(`
                    ...
                    // Update page counters or other UI elements if needed
                    updatePageCounters();
...
// Update page counters (you can expand this based on your UI)
function updatePageCounters() {
    // Update item count display if it exists
    const $itemCount = $('.text-muted').filter(':contains("Showing")');
    if ($itemCount.length) {
        // Refresh the pagination info via AJAX if needed
        // For now, we'll just let the auto-refresh handle it
    }
}

// Enhanced edit modal for AJAX updates
        ...
        success: function(response) {
            if (response.success) {
                ...
                if (response.data && response.data.item_id) {
                    updateItemInList(response.data.item_id, response.data);
                }
...
// Update item in the list with new data
function updateItemInList(itemId, data) {
    const $item = $('#pledge-' + itemId);
    if ($item.length && data) {
        // Update donor name
        if (data.donor_name) {
            $item.find('.donor-name').first().text(data.donor_name);
        }
        // Update amount
        if (data.amount) {
            $item.find('.amount').text('£' + Math.round(data.amount));
        }
        // Update phone
        if (data.donor_phone) {
            $item.find('.donor-phone').text(data.donor_phone);
        }
        
        // Add a brief highlight effect
        $item.addClass('bg-light').delay(1000).queue(function() {
