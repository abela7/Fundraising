...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_system_info') {
    ...
    // Only process if at least one field is provided
    $has_login_count = isset($_POST['login_count']) && $_POST['login_count'] !== '';
    $has_last_login = isset($_POST['last_login_at']) && $_POST['last_login_at'] !== '';
    
    if (!$has_login_count && !$has_last_login) {
        $error_message = 'Please provide at least one field to update.';
    } else {
        $login_count = $has_login_count ? (int)$_POST['login_count'] : null;
        $last_login_at = $has_last_login ? trim($_POST['last_login_at']) : null;
        
        // Validate login_count
        if ($login_count !== null && $login_count < 0) {
            $error_message = 'Login count cannot be negative.';
        } else {
        ...
        if ($db_connection_ok) {
            try {
                ...
                if ($login_count !== null) {
                    $update_fields[] = "login_count = ?";
                    $update_types .= "i";
                    $update_params[] = $login_count;
                }
                
                if ($last_login_at !== null && $last_login_at !== '') {
                    // Validate date format - handle both datetime-local format (YYYY-MM-DDTHH:mm) and MySQL format
                    if (strpos($last_login_at, 'T') !== false) {
                        // datetime-local format: convert to MySQL datetime
                        $date_obj = DateTime::createFromFormat('Y-m-d\TH:i', $last_login_at);
                        if ($date_obj === false) {
                            $date_obj = DateTime::createFromFormat('Y-m-d\TH:i:s', $last_login_at);
                        }
                        if ($date_obj !== false) {
                            $last_login_at = $date_obj->format('Y-m-d H:i:s');
                        } else {
                            throw new RuntimeException('Invalid date format for last login.');
                        }
                    } else {
                        // Already in MySQL format, validate it
                        $date_obj = DateTime::createFromFormat('Y-m-d H:i:s', $last_login_at);
                        if ($date_obj === false) {
                            throw new RuntimeException('Invalid date format for last login.');
                        }
                    }
                    ...
                    $update_types .= "s";
                    $update_params[] = $last_login_at;
                } elseif ($has_last_login && $last_login_at === '') {
                    // Clear last_login_at if empty string sent - handle NULL separately
                    $update_fields[] = "last_login_at = NULL";
                }
                
                // Check if we have anything to update (more than just updated_at)
                $has_updates = !empty($update_params) || (count($update_fields) > 1 && in_array("last_login_at = NULL", $update_fields));
                
                if (!$has_updates) {
                    $error_message = 'No fields to update.';
                } else {
                    // Build SQL - handle NULL separately
                    if (!empty($update_params)) {
                        ...
                        // Only NULL updates, no parameters needed except ID
                        $sql = "UPDATE donors SET " . implode(", ", $update_fields) . " WHERE id = ?";
                        $update_stmt = $db->prepare($sql);
                    ...
                    if (!$update_stmt->execute()) {
                        throw new RuntimeException('Failed to update system information: ' . $update_stmt->error);
                    }
                    $update_stmt->close();
                    
                    // Audit log
                    $beforeData = [
                        'login_count' => $donor['login_count'] ?? 0,
                        'last_login_at' => $donor['last_login_at'] ?? null
                    ];
                    
                    $afterData = [
                        'login_count' => $login_count !== null ? $login_count : ($donor['login_count'] ?? 0),
                        'last_login_at' => $last_login_at !== null ? $last_login_at : ($donor['last_login_at'] ?? null)
                    ];
                ...
                $db->rollback();
                $db->autocommit(true);
                error_log('System info update error: ' . $e->getMessage());
                $error_message = 'Error updating system information. Please try again.';
            }
        }
    }
}
                ...
                <div class="row g-4">
                    ...
                    <div class="col-lg-4">
                        ...
                        <div class="card mb-4">
                            <div class="card-header">
                                <h5 class="card-title">
                                    <i class="fas fa-server text-primary"></i>System Information
                                </h5>
                            </div>
                            <div class="card-body">
                                <form method="POST" id="systemInfoForm">
                                    ...
                                    <div class="mb-3">
                                        <label class="form-label fw-bold">
                                            <i class="fas fa-sign-in-alt me-2"></i>Login Count
                                        </label>
                                        <input type="number" 
                                               class="form-control" 
                                               name="login_count" 
                                               value="<?php echo (int)($donor['login_count'] ?? 0); ?>"
                                               min="0"
                                               placeholder="0">
                                        <div class="form-text">Total number of portal logins</div>
                                    </div>
                                    
                                    <!-- Last Login At -->
                                    <div class="mb-3">
                                        <label class="form-label fw-bold">
                                            <i class="fas fa-clock me-2"></i>Last Login At
                                        </label>
                                        <input type="datetime-local" 
                                               class="form-control" 
                                               name="last_login_at" 
                                               value="<?php 
                                                   if (!empty($donor['last_login_at'])) {
                                                       $date = new DateTime($donor['last_login_at']);
                                                       echo $date->format('Y-m-d\TH:i');
                                                   }
                                               ?>"
                                               placeholder="Never">
                                        <div class="form-text">Last portal login timestamp (leave empty to clear)</div>
                                    </div>
                                    
                                    <!-- Portal Token (Read-only display) -->
                                    <?php if (!empty($donor['portal_token'])): ?>
                                    <div class="mb-3">
                                        <label class="form-label fw-bold text-muted">
                                            <i class="fas fa-key me-2"></i>Portal Token
                                        </label>
                                        <div class="input-group">
                                            <input type="text" 
                                                   class="form-control font-monospace" 
                                                   value="<?php echo htmlspecialchars(substr($donor['portal_token'], 0, 20) . '...'); ?>"
                                                   readonly>
                                            <button type="button" 
                                                    class="btn btn-outline-secondary" 
                                                    onclick="copyToClipboard('<?php echo htmlspecialchars($donor['portal_token']); ?>', this)"
                                                    title="Copy full token">
                                                <i class="fas fa-copy"></i>
                                            </button>
                                        </div>
                                        <div class="form-text">Token for portal access (read-only)</div>
                                    </div>
                                    <?php endif; ?>
                                    
                                    <!-- Token Generated At (Read-only) -->
                                    <?php if (!empty($donor['token_generated_at'])): ?>
                                    <div class="mb-3">
                                        <label class="form-label text-muted">Token Generated At</label>
                                        <p class="mb-0">
                                            <?php 
                                            $date = new DateTime($donor['token_generated_at']);
                                    ...
                                    <!-- Token Expires At (Read-only) -->
                                    <?php if (!empty($donor['token_expires_at'])): ?>
                                    <div class="mb-3">
                                        <label class="form-label text-muted">Token Expires At</label>
                                        <p class="mb-0">
                                            <?php 
                                            $date = new DateTime($donor['token_expires_at']);
                                            echo $date->format('d M Y, H:i');
                                            ?>
                                        </p>
                                    </div>
                                    ...
                                    <?php if (!empty($donor['created_at'])): ?>
                                    <div class="mb-3">
                                        <label class="form-label text-muted">Account Created</label>
                                        <p class="mb-0">
                                            <?php 
                                            $date = new DateTime($donor['created_at']);
                                            echo $date->format('d M Y, H:i');
                                            ?>
                                        </p>
                                    </div>
                                    ...
                                    <?php if (!empty($donor['updated_at'])): ?>
                                    <div class="mb-3">
                                        <label class="form-label text-muted">Last Updated</label>
                                        <p class="mb-0">
                                            <?php 
                                            $date = new DateTime($donor['updated_at']);
                                            echo $date->format('d M Y, H:i');
                                            ?>
                                        </p>
                                    </div>
                                    ...
                                    <div class="d-grid">
                                        <button type="submit" class="btn btn-primary">
                                            <i class="fas fa-save me-2"></i>Update System Info
                                        </button>
                                    </div>
                                </form>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle datetime-local input conversion for last_login_at
    const lastLoginInput = document.querySelector('input[name="last_login_at"]');
    if (lastLoginInput) {
        const systemInfoForm = document.getElementById('systemInfoForm');
        if (systemInfoForm) {
            systemInfoForm.addEventListener('submit', function(e) {
                // Convert datetime-local format to MySQL datetime format
                if (lastLoginInput.value) {
                    // datetime-local returns YYYY-MM-DDTHH:mm, convert to YYYY-MM-DD HH:mm:ss
                    const dateValue = lastLoginInput.value;
                    lastLoginInput.value = dateValue + ':00';
                }
            });
        }
    }
