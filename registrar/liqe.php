...
// Calculate next tombola code for Liqe Liqawunt
$minCode = 335;
$maxCode = 344;
$nextCode = sprintf('%04d', $minCode); // Default

if ($db_connection_ok) {
    // Find highest used code by Liqe in pledges
    $stmt = $db->prepare("SELECT MAX(CAST(notes AS UNSIGNED)) as max_code FROM pledges WHERE created_by_user_id = ? AND CAST(notes AS UNSIGNED) >= ? AND CAST(notes AS UNSIGNED) <= ?");
    $stmt->bind_param("iii", $user['id'], $minCode, $maxCode);
    $stmt->execute();
    $res = $stmt->get_result()->fetch_assoc();
    $max_pledge = $res && $res['max_code'] ? (int)$res['max_code'] : 0;
    $stmt->close();
    
    // Find highest used code by Liqe in payments
    $stmt = $db->prepare("SELECT MAX(CAST(reference AS UNSIGNED)) as max_code FROM payments WHERE received_by_user_id = ? AND CAST(reference AS UNSIGNED) >= ? AND CAST(reference AS UNSIGNED) <= ?");
    $stmt->bind_param("iii", $user['id'], $minCode, $maxCode);
    $stmt->execute();
    $res2 = $stmt->get_result()->fetch_assoc();
    $max_payment = $res2 && $res2['max_code'] ? (int)$res2['max_code'] : 0;
    $stmt->close();
    
    $currentMax = max($max_pledge, $max_payment);
    if ($currentMax > 0 && $currentMax < $maxCode) {
        $nextCode = sprintf('%04d', $currentMax + 1);
    } elseif ($currentMax >= $maxCode) {
        $nextCode = ''; // Out of codes
    }
}
$pkgRows = [];
if ($db_connection_ok) {
